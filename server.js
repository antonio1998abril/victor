...
function getremind(time) {
  try {
    const getAct = async () => {
      let ListReminder = await Activities.find({ Status: false })
        .lean()
      ...
      Remember2 =Remember2.getHours().toString().padStart(2, "0") + ":" + Remember2.getMinutes().toString().padStart(2, "0");
       /*              
                } else if (State.numbertype == 2 && State.Status == false) {
                ...
                } else if (State.numbertype == 2 && State.Status == true) {
                  let notifications = new Notification({
      ...
      if (ListReminder.length >= 0) {
        for (i = 0; i < ListReminder.length; i++) {
          ...
          Medics = ListReminder[i].paciente_id.MedicoDeCabecera;
    ...
    /*           for (i = 1; i <= Medics.length; i++) { 
            if(Medics[i]) {
              var medic = Medics[i].id
              sendNotification({medic,State}) 
               } 
            }  */
            console.log(State, Medics)
           sendNotification({Medics,State})  
          }
        }
